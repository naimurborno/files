...
# UGILE (SDXL port) import — graceful fallback if file not yet on path
try:
    from latent_escape_sampler_sdxl import run_sdxl_ugile as _run_sdxl_ugile
    _UGILE_SDXL_AVAILABLE = True
except ImportError:
    _UGILE_SDXL_AVAILABLE = False
...
def main():
    ...
    # Register UGILE (SDXL port) runner if available
    if _UGILE_SDXL_AVAILABLE:
        MODEL_REGISTRY["sdxl_ugile"] = _run_sdxl_ugile
    ...
    if model_name in ("sd3_onlb", "sd3_peakback", "sd3_ugile", "sana_ugile", "lumina2_ugile",
                      "pixart_sigma_ugile", "sdxl_ugile"):
        seeds = opts["seeds"]
