"""
latent_escape_sampler_sdxl.py
--------------------------------
UGILE ported to SDXL. Same algorithm as SD3/SANA/Lumina2/PixArt-Sigma:
...
SDXL is a STANDARD eps-prediction UNet diffusion model — same category as
PixArt-Sigma, none of Lumina2's quirks apply:
  - no reversed timestep
  - no output negation
  - no cfg_trunc_ratio / cfg_normalization
  - sigma_k read straight from scheduler.sigmas (EulerDiscreteScheduler
    exposes this natively, same call site as the PixArt-Sigma port)

Only _velocity_forward changes vs. PixArt-Sigma:
  - two-encoder text embeds + pooled embeds instead of T5 + attention_mask
  - added_cond_kwargs is REQUIRED every step (text_embeds/time_ids), not
    optional/-MS-only
  - no learned-variance channel split on the UNet output (SDXL's UNet always
    predicts eps only, out_channels == in_channels)
"""
...
class SDXLUGILESampler:
    ...
    def __init__(
        ...
        scheduler,
        cfg             : dict,
        wrapper         = None,          # needed for added_cond_kwargs (pooled embeds, H, W)
        device          : str   = "cuda",
        sigma_lo        : float = 0.3,
        sigma_hi        : float = 0.9,
        theta_max       : float = 0.5,
        ...
        self.guidance_scale = f_cfg.get("guidance_scale", 5.0)
        self.do_cfg         = self.guidance_scale > 1.0

        gen_cfg = cfg.get("generation", {})
        self.H  = gen_cfg.get("height", 1024)
        self.W  = gen_cfg.get("width",  1024)

    # ================================================================== #
    #  PUBLIC ENTRY                                                        #
    # ================================================================== #

    def run(self, x0, text_embeddings, pooled_embeds, seed: int = 0) -> Dict[str, Any]:
        cache = self._forward_pass_with_profiling(x0, text_embeddings, pooled_embeds)
        ...
        x_N_diverse = self._full_forward_pass(x0_new, text_embeddings, pooled_embeds)
    ...
    def _forward_pass_with_profiling(self, x0, text_embeddings, pooled_embeds):
        ...
        for k, t in enumerate(timesteps):
            v, v_uncond, v_cond = self._velocity_forward(
                x, t, text_embeddings, pooled_embeds, return_split=True
            )
            ...
            if hasattr(self.scheduler, "sigmas"):
                cached_sigma[k] = max(self.scheduler.sigmas[k].item(), 1e-4)
            else:
                cached_sigma[k] = max(1.0 - k / N, 1e-4)
    ...
    def _full_forward_pass(self, x0_new, text_embeddings, pooled_embeds):
        self.scheduler.set_timesteps(self.num_steps, device=self.device)
        timesteps = self.scheduler.timesteps
        x = x0_new.clone()
        for t in timesteps:
            v = self._velocity_forward(x, t, text_embeddings, pooled_embeds)
            x = self.scheduler.step(v, t, x).prev_sample
        return x
    ...
    def _velocity_forward(self, x, t, text_embeddings, pooled_embeds, return_split=False):
        ...
        # UNet2DConditionModel broadcasts a scalar/0-d timestep across the
        # batch internally — unlike the PixArt-Sigma transformer, no manual
        # t.expand(batch) is needed here.

        # SDXL requires added_cond_kwargs (text_embeds/time_ids) on EVERY
        # step, unlike PixArt-Sigma where it's only needed for -MS
        # checkpoints. wrapper is mandatory here (not Optional in practice).
        added = self.wrapper.added_cond_kwargs(
            pooled_embeds, latent_input.shape[0], self.H, self.W, dtype, device
        )
        ...
        with torch.no_grad():
            ...
            # SDXL's UNet always predicts eps only (out_channels ==
            # in_channels) — no learned-variance split needed here, unlike
            # PixArt-Sigma's -MS checkpoints.
...
def run_sdxl_ugile(opts: dict):
    from pipeline_wrapper_sdxl import SDXLPipelineWrapper
    ...
    if prompts_file:
        ...
        print(f"[UGILE-SDXL] Loaded {len(prompts)} prompt(s) from {prompts_file}")
    else:
        prompts = cfg.get("prompts") or [opts["prompt"]]

    # Global position of the FIRST prompt in this process's slice, within the
    # full prompts file — see the matching comment in
    # latent_escape_sampler_pixart_sigma.py's run_pixart_sigma_ugile.
    prompt_offset = opts.get("prompt_offset", cfg.get("prompt_offset", 0))

    print(f"[UGILE-SDXL] Loading model…")
    wrapper = SDXLPipelineWrapper(cfg, device=device)
    wrapper.load()

    sampler = SDXLUGILESampler(
        unet         = wrapper.unet,
    ...
    for local_idx, prompt in enumerate(prompts):
        ...
        print(f"\n[UGILE-SDXL] Prompt {p_idx + 1} (local {local_idx + 1}/{len(prompts)}): \"{prompt}\"")
        prompt_embeds, pooled_embeds = wrapper.encode_prompt(prompt, opts["negative_prompt"])

        for seed in seeds:
            print(f"[UGILE-SDXL]   seed={seed}")
    ...
    print(f"\n[UGILE-SDXL] Done — {len(records)} image(s)")
    return records
