...
class UGILESampler:
    ...
    def __init__(
        ...
        gamma           : float = 1.2,
        xi_mix          : float = 0.80,  # low-freq/high-freq blend for the escape-direction
                                          # noise xi: xi = xi_mix*xi_low + (1-xi_mix)*xi_high
    ):
        self.unet           = unet
    ...
    def run(
        ...
        if x0.dim() == 4:
            ...
            # xi_mix fraction low-freq (pose) + (1 - xi_mix) high-freq (texture diversity)
            # controlled via cfg -> ugile.xi_mix (default 0.80)
            xi = self.xi_mix * xi_low + (1.0 - self.xi_mix) * xi
