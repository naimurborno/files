...
class Lumina2PipelineWrapper:
    ...
    def load(self):
        ...
        # hardcoding here, same caution as the SANA wrapper. Configurable
        # via cfg["vae"]["scale_factor"] for experimentation; defaults to
        # the pipeline's own value if not set.
        vae_cfg = self.cfg.get("vae", {})
        self.vae_scale_factor = vae_cfg.get("scale_factor") or self.pipe.vae_scale_factor
    ...
    def decode_latents(self, latents: torch.Tensor) -> Image.Image:
        ...
        # Both overridable via cfg["vae"] for sweep/experimentation use;
        # fall back to the checkpoint's own vae.config values otherwise.
        vae_cfg = self.cfg.get("vae", {})
        scaling_factor = vae_cfg.get("scaling_factor")
        if scaling_factor is None:
            scaling_factor = self.vae.config.scaling_factor
        shift_factor = vae_cfg.get("shift_factor")
